export default async function handler(req, res) {
  ...
  try {
    ...
    const mapped = items.map(item => ({ ...item.raw_data, bidNtceNo:item.bid_ntce_no, bidNtceNm:item.bid_ntce_nm, ntceInsttNm:item.ntce_instt_nm, dminsttNm:item.dminstt_nm, bidMethdNm:item.bid_methd_nm, asignBdgtAmt:item.asign_bdgt_amt, ntceDt:item.ntce_dt, bidClseDt:item.bid_cls_dt }));
