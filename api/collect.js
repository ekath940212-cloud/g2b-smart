...
function parseDate(str) {
  ...
  for (const { ep, type } of endpoints) {
    try {
      const firstUrl = buildUrl(ep, G2B_API_KEY, sd, ed, 1, 100);
      const firstData = await fetch(firstUrl).then(r=>r.json());
      const totalCount = firstData && firstData.response && firstData.response.body ? firstData.response.body.totalCount || 0 : 0;
      const totalPages = Math.ceil(totalCount / 100);
      const pagePromises = [];
      for (let p = 1; p <= Math.min(totalPages, 20); p++) {
        pagePromises.push(
          fetch(buildUrl(ep, G2B_API_KEY, sd, ed, p, 100))
            .then(r=>r.json())
            .then(d => {
              if (!d || !d.response || !d.response.body) return [];
              const raw = d.response.body.items;
              if (!raw) return [];
              return Array.isArray(raw) ? raw : [raw];
            })
            .catch(()=>[])
...
function buildUrl(ep, apiKey, sd, ed, page, rows) {
  const url = new URL('https://apis.data.go.kr/1230000/ad/BidPublicInfoService/' + ep);
  url.searchParams.set('serviceKey', apiKey); url.searchParams.set('numOfRows', String(rows));
  url.searchParams.set('pageNo', String(page)); url.searchParams.set('inqryDiv', '1');
  url.searchParams.set('type', 'json'); url.searchParams.set('inqryBgnDt', sd); url.searchParams.set('inqryEndDt', ed);
  return url.toString();
}
...
function parseDate(str) {
  ...
  for (const { ep, type } of endpoints) {
    try {
      ...
      const pagePromises = [];
      for (let p = 1; p <= Math.min(totalPages, 20); p++) {
        pagePromises.push(fetch(buildUrl(ep, G2B_API_KEY, sd, ed, p, 100)).then(r=>r.json()).then(d=>{ const items=d?.response?.body?.items?.item; return !items?[]:Array.isArray(items)?items:[items]; }).catch(()=>[]));
      }
      const items = (await Promise.all(pagePromises)).flat();
      for (let i = 0; i < items.length; i += 100) {
        const batch = items.slice(i, i+100).map(item => ({
