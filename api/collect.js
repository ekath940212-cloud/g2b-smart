export default async function handler(req, res) {
  ...
  if (req.query.startDt && req.query.endDt) {
    ...
    startDt = fmt(past); endDt = fmt(now);
  }
  const chunks = [];
  ...
  res.status(200).json({ success:true, totalSaved, errors, collectedAt: new Date().toISOString(), range: { startDt, endDt }, chunks: chunks.length });
}
